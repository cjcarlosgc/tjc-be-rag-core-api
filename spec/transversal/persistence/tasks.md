...
- [ ] **(HU27/HU28)** Migrar `ContextTrace` y almacenamiento paginable de archivos descubiertos conforme a `011-context-traces`, sin duplicar archivos completos; incluir constraints e índices de foreign keys/listados.
- [ ] **(HU29)** Agregar `Project.ownerUserId` UUID, índice y estrategia explícita para datos live preexistentes; no asignar propietario silenciosamente. Revisar privilegios Data API/RLS y probar aislamiento entre propietarios.
- [x] **(HU62, seguridad de datos)** RLS en todas las tablas y revocación de privilegios a `anon`/`authenticated`: migración `20260921130000_enable_rls_all_tables` (escrita y **NO aplicada a Supabase**; se aplica al desplegar el bundle A) y prueba de guardia `app/src/prisma/rls-guard.spec.ts` (falla si una tabla de `schema.prisma` no habilita RLS en alguna migración). Evidencia: migración validada en un PostgreSQL 14 local descartable, con y sin roles `anon`/`authenticated` (idempotente al aplicarla dos veces; `anon` recibe `permission denied` en las 20 tablas y en `_prisma_migrations`; una tabla futura del rol propietario no se concede a `anon`; el rol propietario sigue leyendo y escribiendo; con solo RLS y un `GRANT` explícito `anon` ve 0 filas). Pendiente del usuario: desactivar la Data API en el panel de Supabase.
