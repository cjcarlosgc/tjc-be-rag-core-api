...
- Acceso por rol (`INTEROP-2.4` §6.13, soporte de HU01/HU02): un recurso no visible conserva su `404` y uno visible con rol insuficiente es `403 PROJECT_ROLE_INSUFFICIENT` (`details: { requiredRole, currentRole }`). Códigos: `GITHUB_IDENTITY_REQUIRED` (401), `IDENTITY_UNAVAILABLE` (503), `WORKSPACE_NOT_FOUND` (404), `WORKSPACE_ADMIN_REQUIRED` (403), `REPOSITORY_OUTSIDE_WORKSPACE` (400), `REPOSITORY_PERMISSION_INSUFFICIENT` (403) y `GITHUB_VERIFICATION_UNAVAILABLE` (503). Una caída de GitHub al verificar un acceso nuevo es `503`, nunca `404` ni `500`; los accesos ya registrados no se revocan por un error de red.
