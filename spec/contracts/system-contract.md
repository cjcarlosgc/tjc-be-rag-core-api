...
- **Prerrequisito de despliegue (decisión del usuario pendiente):** la App es hoy privada (`GET /apps/{slug}` sin autenticación responde `404`) y solo puede instalarse en la cuenta de su dueño, sin ninguna instalación de organización. Para que una organización tenga workspace, la App debe ser instalable por esa organización (pública, o creada dentro de ella) y con `Members: read` aceptado. El ajuste "Install this app" no es visible por API y hay que confirmarlo en la configuración de la App.
