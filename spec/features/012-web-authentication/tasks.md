...
- [x] La tabla `user_github_identities` es la raíz de confianza de la identidad: la migración habilita RLS sin políticas y revoca `anon`/`authenticated` (solo si esos roles existen) para que la Data API de Supabase no la exponga.
- [ ] Verificación de despliegue (pendiente): tras aplicar las migraciones a Supabase y desactivar la Data API, comprobar con la clave `anon` que `GET /rest/v1/user_github_identities` (y el resto de tablas) no devuelve filas.
