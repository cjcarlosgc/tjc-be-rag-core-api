...
- Al crear un `Project`, Core persiste como creador el `sub` validado del token. Hasta `T-003` todo recurso descendiente se autorizaba recorriendo esa propiedad; con `014-organizations-access` (HU58-HU64) se autoriza por el rol de la persona sobre el Project, derivado de GitHub, y en los Projects de organización `ownerUserId` queda como dato del creador que no autoriza por sí solo; en un Project personal sigue siendo la única autorización (solo su creador lo ve, siempre como Admin).
...
- La política de provisión puede ser por invitación o solicitud de acceso empresarial. HU29 no aprueba auto-registro público, roles múltiples, organizaciones ni proyectos compartidos; estos últimos los introduce `014-organizations-access` (`DEC-ORG-001`), solo para organizaciones: un Project personal sigue siendo solo de su creador.
