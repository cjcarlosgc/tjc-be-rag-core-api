...
- SYSTEM-2.4 e INTEROP-2.4 (§6.1, §6.8, §6.9, §6.13); `DEC-ORG-001` y `DEC-ORG-002` APROBADOS (este último enmienda la visibilidad personal de `DEC-ORG-001`).
...
2. **HU63 + HU58.** `GET /workspaces`, `workspaceId` en `POST /projects`, `GET /projects` y `GET /integrations/github/repositories`, `PATCH /projects/{id}`, `workspace` y `role` en `ProjectResponse`, columnas de organización. Interino hasta el corte 3: el predicado sigue siendo el creador (`ownerUserId`) y `role` es `ADMIN`, que ya es el comportamiento final para Projects personales; `POST /projects` en una organización ya verifica en vivo que el usuario es owner, pero un Project de organización solo lo ve su creador hasta el corte 3. Nada de esto comparte Projects todavía.
3. **HU59 + HU60.** `project_access` (solo Projects de organización), `accessibleProject` (rama personal por `ownerUserId`, rama de organización por registro), `ProjectAccessService`, alta al entrar, roles Admin/Maintainer/Reader, `403 PROJECT_ROLE_INSUFFICIENT` en toda la matriz, listados omitiendo lo no verificable, `503 GITHUB_VERIFICATION_UNAVAILABLE`, WebSocket. Migra todos los repositorios que usan `ownedProject`.
4. **HU64.** Filtro por workspace en el discovery, `REPOSITORY_OUTSIDE_WORKSPACE`, `REPOSITORY_PERMISSION_INSUFFICIENT`, orden de validación de `POST .../integrations/github` y permiso mínimo `maintain`/`write`/`admin` en `verify-app-access`/`branches` (`DEC-ORG-002`, corrección de seguridad).
...
- **Corte 3:** matriz e2e completa de INTEROP §6.13 por cada ruta y cada rol (`404` sin visibilidad, `403` con rol insuficiente, `2xx` con el mínimo), jerarquía, alta al entrar por deep link a un Project de organización, Project sin repositorio visible solo a Admin, binding `REVOKED` visible solo a Admin, repositorio público sin membresía activa, Project personal invisible (`404`) para colaboradores del repositorio y ausente de `GET /projects` de otros, Project personal que no depende de GitHub, GitHub caído (accesos existentes vigentes, nuevos `503`), WebSocket con y sin rol.
- **Corte 4:** filtro por workspace, permiso mínimo en `verify-app-access` y `branches` (`403`/`NOT_AUTHORIZED`/`404`), orden de validación completo de `POST .../integrations/github`, sin sondeo de repositorios ajenos (`REPOSITORY_ALREADY_BOUND` solo tras las validaciones de propietario y permiso), no revinculación.
