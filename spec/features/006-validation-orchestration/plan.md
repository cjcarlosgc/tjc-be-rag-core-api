...
`TestExecutionService -> ObjectStorageService.presignGet -> SandboxExecutionService -> HTTP tjc-be-test-execution-sandbox`. Mantener `202 + polling`, Bearer e idempotencia conforme a `INTEROP-2.1`; enviar referencias de descarga efímeras verificables, mapear `SandboxExecutionResultResponse` y persistirlo en Core sin signed URLs. El adapter deriva una identidad UUID v5 por unidad lógica y reutiliza ese valor en `requestId`/`Idempotency-Key` durante retries.
