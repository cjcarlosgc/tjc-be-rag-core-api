...
**No existe verificación end-to-end contra un Sandbox real.** El cliente HTTP cubre el subconjunto legado de DTOs, correlación, polling, Bearer e identidad idempotente estable ahora preservado por `INTEROP-2.1`, pero no se probó contra el servicio real desplegado. `evidenceIds` en `ValidationResponse` queda siempre `[]`: la captura/almacenamiento de evidencia no se implementó en ese corte.
