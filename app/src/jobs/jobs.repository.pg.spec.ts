 ...
 * Supabase). La URL debe apuntar a localhost: la suite VACÍA la tabla `jobs`. Las columnas son
 * `timestamp` sin zona que Prisma escribe en UTC y la cola las compara con `now() AT TIME ZONE
 * 'utc'`, así que NO depende de la zona de la sesión: la suite se ejecuta con la sesión en `UTC` y
 * en `America/Bogota` (opción `-c timezone=` de la conexión).
 *
...
const ZONES = ['UTC', 'America/Bogota'];

for (const zone of ZONES) {
  describe.skipIf(!url)(`JobsRepository against a local PostgreSQL (dedupeKey, HU61) [session time zone ${zone}]`, () => {
    ...
    beforeAll(() => {
      ...
      // La zona horaria de la SESIÓN se fija por la conexión: la cola no debe depender de ella.
      const connection = new URL(url as string);
      connection.searchParams.set('options', `-c timezone=${zone}`);
      prisma = new PrismaClient({ adapter: new PrismaPg({ connectionString: connection.toString() }) });
    ...
    it('the session really runs in the zone under test', async () => {
      const [row] = await prisma.$queryRaw<Array<{ timezone: string }>>`SELECT current_setting('timezone') AS timezone`;
    ...
    const age = (id: string, seconds: number) =>
      prisma.$executeRaw`UPDATE "jobs" SET "lockedAt" = (now() AT TIME ZONE 'utc') - make_interval(secs => ${seconds}::double precision) WHERE "id" = ${id}`;
    ...
    describe('deduplication only over PENDING', () => {
      ...
      it('schedules the new job in the future using the database clock', async () => {
        ...
        const [row] = await prisma.$queryRaw<Array<{ ahead: number }>>`
          SELECT extract(epoch FROM ("availableAt" - (now() AT TIME ZONE 'utc')))::float AS ahead FROM "jobs"`;
