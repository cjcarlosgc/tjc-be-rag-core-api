...
3. **Post-tarea:** PULL `before-review`, revisión independiente y contract-reviewer si afecta contrato; roles compatibles pueden revisar en paralelo. Por defecto, antes de declarar el WI terminado, el usuario revisa el corte: leader presenta diff, criterios de aceptación, pruebas y evidencia, y espera su veredicto. Solo si el usuario lo solicita explícitamente, leader delega la revisión independiente a un agente `reviewer`. Leader consolida el fan-in, corrige hallazgos (máximo dos ciclos), hace PULL `before-done`, valida gates y registra el cierre. Después hay revisión acumulada del rango antes de cualquier push solicitado.

Las asignaciones pueden quedar vacías hasta que se designen sus responsables. Al entrar en `W-IN_REVIEW`, `implementationAgent` debe identificar al implementer y `reviewAgent` al revisor independiente: usa `human-reviewer` por defecto y `reviewer` si el usuario delegó explícitamente en el agente. El implementer nunca puede revisarse a sí mismo. Una delegación de revisión no sustituye la aprobación humana de alcance/arquitectura exigida en `W-AWAITING_APPROVAL`.
...
Los valores son `G-NOT_RUN`, `G-PASSED`, `G-FAILED` y `G-NOT_APPLICABLE`. Para `W-DONE` deben pasar `sddVerified`, `implementationCompleted`, `independentReviewPassed`, `technicalChecksPassed`, `interopSyncChecked`, `noBlockingDecisions` y `retryLimitRespected`. Si `contractImpact=true`, también `contractReviewed` y `canonicalContractSynced`. `contractSyncPublished` pasa solo cuando `publishesContract=true` y hay un evento en outbox; los demás casos usan `G-NOT_APPLICABLE`. Cada gate necesita evidencia reproducible, no solo un estado declarado.
...
node harness/contract-sync.mjs check --checkpoint start --work-item WI-CORE-001
```
